...
Covers:
- Core ``expand_to_span_trajectories`` transform (Q1: lives in shortchain.data)
...
def sample_trajectory() -> Trajectory:
    """A 3-span trajectory for testing."""
    return Trajectory(
        task_id="task_001",
        intent="Find the weather forecast for tomorrow",
        spans=[
            Span(action="search_api(query='weather')", observation="results"),
            Span(action="parse_json(data='{}')", observation="parsed"),
            Span(action="format_response(text='sunny')", observation="done"),
        ],
        success=True,
...
def failed_trajectory() -> Trajectory:
    ...
    return Trajectory(
        ...
        spans=[
            Span(action="flight_search(dest='NYC')", observation="error"),
        ],
        success=False,
        app_name="travel_app",
    )
...
# ---------------------------------------------------------------------------
# Tests: expand_to_span_trajectories (Q1 — core transform)
# ---------------------------------------------------------------------------

class TestExpandToStepTrajectories:
    """Tests for the core span-expansion transform."""

    def test_expands_correct_count(self, sample_trajectory):
        """Each span with a tool produces one sub-trajectory."""
        expanded = expand_to_span_trajectories(sample_trajectory)
        assert len(expanded) == 3

    def test_span_ids_are_unique(self, sample_trajectory):
        expanded = expand_to_span_trajectories(sample_trajectory)
    ...
    def test_each_span_has_single_tool(self, sample_trajectory):
        expanded = expand_to_span_trajectories(sample_trajectory)
        for sub in expanded:
            assert len(sub.tools_used) == 1

    def test_span_0_has_correct_tool(self, sample_trajectory):
        expanded = expand_to_span_trajectories(sample_trajectory)
        assert expanded[0].tools_used == {"search_api"}

    def test_span_2_has_all_prior_spans(self, sample_trajectory):
        expanded = expand_to_span_trajectories(sample_trajectory)
        last = expanded[2]
        # Should contain spans 0, 1, 2
        assert len(last.spans) == 3

    def test_metadata_has_span_info(self, sample_trajectory):
        expanded = expand_to_span_trajectories(sample_trajectory)
        meta = expanded[1].metadata
        assert meta["span_index"] == 1
        assert meta["total_spans"] == 3
    ...
    def test_empty_trajectory_returns_empty(self):
        traj = Trajectory(task_id="empty", intent="nothing", spans=[])
        assert expand_to_span_trajectories(traj) == []

    def test_skips_spans_without_tools(self):
        traj = Trajectory(
            task_id="mixed",
            intent="test",
            spans=[
                Span(action="tool_a()", observation="ok"),
                Span(action=None, observation="no tool"),  # no tool
                Span(action="tool_b()", observation="ok"),
            ],
        )
        expanded = expand_to_span_trajectories(traj)
...
class TestToolBenchAdapter:
    ...
    def test_span_level_expansion(self, trajectory_file):
        config = BenchmarkConfig(span_level=True)
        adapter = ToolBenchAdapter(
            benchmark_config=config,
            train_path=trajectory_file,
        )
        train = adapter.load_trajectories("train")
        # With span expansion, we should have more samples than trajectories
        # The successful trajectory has 3 spans → 3 sub-trajectories
        assert len(train) == 3
...
class TestBenchmarkConfig:
    ...
    def test_default_benchmark_config(self):
        ...
        assert cfg.benchmark.span_level is False
...
"""
        ...
        assert cfg.benchmark.span_level is True
