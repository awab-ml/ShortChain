...
class TestOpenEnd:
    def test_root_ended_with_success_and_same_trace_as_child(self, otel_global_clear):
    ...
    def test_end_without_success_ends_but_no_success_attr(self, otel_global_clear):
        memory = otel_global_clear
        ts.open_task("t2")
    ...
    def test_set_success_false_is_known_failure(self, otel_global_clear):
        memory = otel_global_clear
        ts.open_task("t3")
    ...
    def test_end_without_task_is_noop(self, otel_global_clear):
...
class TestNested:
    def test_nested_set_task_ends_previous_without_success(self, otel_global_clear):
        memory = otel_global_clear
        ts.open_task("outer")
        ts.open_task("inner")
        ts.end_task(success=True)
    ...
    def test_handle_cleared_after_end(self, otel_global_clear):
        ts.open_task("a")
        ts.open_task("b")
        ts.end_task(success=True)
        assert ts.current_task() is None
...
class TestAssociation:
    def test_merge_not_replace(self, otel_global_clear):
    ...
    def test_map_written_onto_open_root(self, otel_global_clear):
        memory = otel_global_clear
        ts.open_task("t9", intent="x")
    ...
    def test_injection_processor_copies_to_children(self, otel_global_clear):
        memory = otel_global_clear
    ...
    def test_association_cleared_after_end(self, otel_global_clear):
        ts.open_task("t11")
        ts.end_task(success=True)
        assert association_values() == {}

    def test_set_task_children_inherit_via_start_span(self, otel_global_clear):
        memory = otel_global_clear
    ...
    def test_set_association_without_task_is_noop(self, otel_global_clear):
