"""Configuration management for TabAgent.

Loads YAML configs and exposes them as validated Pydantic models.
Every module reads its config slice from the root `TabAgentConfig`.
"""
...
class FieldMapConfig(BaseModel):
    """Maps external log field names to internal TabAgent schema names."""
...
class TabAgentConfig(BaseModel):
...
def load_config(config_path: str | Path | None = None) -> TabAgentConfig:
    """Load and validate a TabAgent configuration.
    ...
    return TabAgentConfig.model_validate(base)
