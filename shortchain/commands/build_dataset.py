"""``shortchain dataset`` — build a training dataset from trajectories.
...
tool features are composed by the builders (see ``shortchain/features/``); the
output is ``train.csv``/``test.csv`` split at the task level (no task leaks
across the split).
