"""``shortchain evaluate`` — evaluate a trained model on a test set.
...
command evaluates an already-built dataset; for the full leak-free benchmark
(feature construction from traces, baselines, bootstrap CIs) use the research
harness in ``scripts/run_validation.py`` with ``--level task|span``.
"""
...
def _make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="shortchain evaluate",
        description="Evaluate a trained ShortChain model.",
    )
    parser.add_argument(
        "--model",
        type=str,
